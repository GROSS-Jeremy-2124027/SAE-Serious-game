...
func _on_DiffuArea2D_input_event(viewport, event, shape_idx):
	Input.set_default_cursor_shape(Input.CURSOR_POINTING_HAND)
	if event is InputEventMouseButton:
		if event.is_pressed():
			set_current_color()
			var monoColor = get_parent().get_node("Q1/ColorRect").color
			var multiColor = get_parent().get_node("Q2/ColorRect").color
			next_color()
			while (get_node("ColorRect").visible and (get_node("ColorRect").color == monoColor or get_node("ColorRect").color == multiColor)):
				next_color()
